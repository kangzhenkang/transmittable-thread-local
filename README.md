...
`TTL`从`v2.6.0`开始，加载`Agent`会自动把自己的`Jar`设置到`boot class path`上。

**_注意_**：不能修改下载的`TTL`的`Jar`的文件名（`transmittable-thread-local-2.x.x.jar`）。如果修改了，则需要自己手动通过`-Xbootclasspath JVM`参数来显式配置。

实现是通过`Java Agent`的`manifest`文件属性`Boot-Class-Path`：
...
- [`Java Agent`的`JavaDoc`文档](https://docs.oracle.com/javase/8/docs/api/java/lang/instrument/package-summary.html#package.description)
...
java -javaagent:path/to/ttl-foo-name-changed.jar \
    -javaagent:path/to/ttl-foo-name-changed.jar \
    -cp classes \
    com.alibaba.ttl.threadpool.agent.demo.AgentDemo
```
